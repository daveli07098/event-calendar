...
- refactor(classify): extract shared `classifyBatch` / `classifySingleEvent` to `src/lib/classify-event.ts` — both the Classify tab and the scrape route now use the same prompt, model cascade, and category list (including `ticket`); scrape falls back to the shared classify logic when the main AI prompt doesn't return a category (4a29a2f)
### Fixed
