...
- fix(discounts): **the scanner was inventing offers out of navigation links.** The candidate-link list added for deep links let the model turn sale-menu entries into promotions — nike.com reported "Sale Shoes", "Student Discount" and "Military Discount" at high confidence with no number anywhere on the page. An offer now survives only if it carries a promo code, a digit or 折 in its own text, or digit-bearing evidence quoted from the page; when nothing survives and the headline has no percent or code, the scan honestly reports no discount ([1b96ee1])
- fix(discounts): `www.puma.com` 301-redirects to `about.puma.com`, the corporate site, so every scan was reading a page with no shop content and reporting "No discount found". The final URL after redirects is now checked and the mismatch is named, pointing at the regional store instead ([1b96ee1])
- fix(discounts): a JavaScript-rendered storefront returned almost no text and surfaced as a red failure with an unhelpful message. It now says what is actually wrong and reads amber, while keeping Re-check since another path on the same site may render server-side ([2e81b7e])
### Changed
- change(discounts): **default sources are now chosen by what the scanner can actually read**, not by brand. Measured readable characters after the app's own tag-stripping: Marathon Sports 91k, GigaSports 63k, Skechers HK 5.5k, against nike.com/hk at 76 and hk.puma.com at 36 — both JavaScript shells — and the global nike.com page at 2.3k of US stock. GigaSports runs the same platform as Marathon Sports and is the one working route to adidas markdowns ([e460674])
### Added
- feat(discounts): **scan a page you paste in.** Checked in a real browser: adidas.com, adidas.com.hk and fanatics.com refuse even genuine Chrome once it is automation-controlled ("A security issue was automatically detected" / "Access Denied"), so a headless renderer on the server would be blocked too — this is not a fetching problem and cannot be fixed from our side. hk.puma.com is not blocked at all but is purely client-rendered: a server fetch sees 36 characters where the rendered page carries "AUTUMN SPECIAL! 3件6折 | 滿$1500減$200" and "正價2件8折". The user's own browser can read all of them, so the scan endpoint now accepts pasted page content and runs the same extraction on it — pasted HTML keeps offer deep links, plain text works without them. No fetch, so no SSRF surface; the AI quota gate still applies. Verified end to end: pasting PUMA HK's page produced three offers with the headline correctly read as 40% off, since 3件6折 means paying 60% ([1daec53])
- feat(discounts): **custom sources live on the account instead of the browser.** They were localStorage-only, so they never followed the user to another device and a failed write was swallowed in silence. Added `User.discountSources` (nullable Json, migration `20260920000000_add_user_discount_sources`) with a validated, deduped, 50-capped `GET`/`PUT /api/discounts/sources`. The client renders its local copy first, loads from the server, migrates any local-only sources up once, and falls back to this-device-only storage with a visible note when the server is unreachable — a failed save now rolls back and says so ([66c3d60], [5e173e4])
- feat(discounts): source rows show a path hint beside the hostname so two sources on the same host stay distinguishable ([e460674])
### Maintenance
- `safeFetch` exposes the post-validation final URL for redirect detection; per-hop SSRF re-validation is unchanged and was independently verified against the guard's full threat model ([1b96ee1])
- test: 414 tests (was 379); lint unchanged at the 13/11 baseline

## [2026-09-07] — Session: Event Section review — discount deep links, venue detection, shell UX
### Fixed
- fix(discounts): **"低至3折" was shown as "30% OFF"** — the AI reported the 折 number as if it were percent off; N折 means paying N/10 of the price, so 3折 is up to 70% off and 85折 is 15% off. Normalised on the server (`src/lib/discounts/percent.ts`) so the badge is right regardless of what the model returns; "up to" is only added when 低至/最低 sits directly before the number, 10折 is treated as no discount, and evidence quotes are only consulted when the headline itself yields nothing ([16e479b])
- fix(layout): the theme-boot script (`next/script` `beforeInteractive`, placed under `<html>`) tripped React 19's "Cannot render a sync or defer `<script>` outside the main document" warning dozens of times per load, and a curl of the served HTML showed it was queued via `__next_s.push` inside `<body>` rather than hoisted into `<head>` — so it was not actually parser-blocking. Now a plain inline `<script>` inside an explicit `<head>`; verified on a production build that it sits before `<body>` with no queue wrapper. Warning gone in dev ([1084cd4])
- fix(venues): removing a venue from the **shared** `EventVenue` directory was one click, no confirmation, and the control was hover-only so it could not be reached on touch. Now behind `AlertDialog` and visible on small screens; same visibility fix for image deletion ([8aacb0d])
- fix(tickets): tab clicks never wrote `?section=`, so reload/back lost the tab; the mobile tab strip clipped the fifth tab with no affordance; no `aria-label` anywhere in the shell; Category Detection swallowed calendar-fetch failures and showed "Loading…" forever. All four addressed ([56c9d90])
### Added
- feat(discounts): **per-offer deep links.** Candidate links are extracted from the raw HTML (the promo menu lives in `<nav>`, which the text extractor strips) and passed to the AI as a numbered list; the model answers with an index per offer, the server maps index→href and refuses anything not on the page. Items are linked to product pages by name match, without AI. Verified live on marathonsports: the clearance offer now links to `/clearance-sale` ([16e479b], [26d83a4])
- feat(discounts): results persist per source in `localStorage` with a "Checked Nm ago" label and stale styling after 24h (a reload used to wipe every result); validity dates render as a chip and flow into "Add to calendar" with the deep link; scan status is announced via `aria-live`; error text is shown in full instead of behind a hover tooltip ([26d83a4])
- feat(venues): **ticket location detection.** `src/lib/venue-match.ts` matches event location text to the venue directory (EN/ZH normalisation, parenthesised nicknames such as 紅館, exact/alias before bounded containment so "PORTAL" does not match "Kowloon Portal Plaza"); `GET /api/venues/events` groups the caller's accessible events by venue with seat text, ticket URL, ticket-calendar flag and seat-map availability, and lists unmatched locations with placeholders ("Hong Kong", "TBD") filtered out. Match-only by design: the directory is shared across users, so nothing is auto-created and `Event.location` is never rewritten ([e7e26f2])
- feat(venues): each venue card shows upcoming / ticketed / past counts and a "Seat map" badge, expands to the event list with Ticket and Open-in-calendar links, and unmatched locations get a human-confirmed "Add to directory" action. The Import result card flags venues that have a seat map ([8aacb0d], [56c9d90])
### Maintenance
- docs: `docs/event-section.md` — scanner pipeline and contract, 折 table, venue matching rules and API shape, theme-boot rationale, test map
- test: 372 tests (was 286); lint unchanged at the 13/11 baseline

## [2026-07-26] — Session: full-project audit, mobile calendar fix, suite repair
### Fixed
- fix(calendar): phones were stuck in `dayGridMonth` with no way out — `isMobile` can only resolve after mount (matchMedia needs `window`; guessing during SSR would desync hydration) but FullCalendar reads `initialView` once at construction, so the mobile default never reached it, and the mobile `headerToolbar` shipped no view-switcher buttons either. The view is now driven imperatively via `getApi().changeView()` on breakpoint resolve/cross, guarded by a `lastBreakpointRef` so a deliberately-chosen view isn't reset by unrelated re-renders or same-side resizes. Mobile also gets a `footerToolbar` switcher (listWeek/dayGridMonth/timeGridDay) on its own row. Covered by 6 regression tests; verified meaningful (4 of 6 fail with the fix stripped) ([c526139])
- fix(calendar): the mobile CSS block was *shrinking* every toolbar button to ~26px at exactly the breakpoint where input is touch — now a 40px minimum, along with the mobile-only sidebar close button, mini-calendar month arrows and day cells, all enlarged on touch only so desktop density is unchanged. 40px is deliberate: past WCAG 2.5.8's 24px floor, while two toolbar rows at a full 44px would eat too much phone viewport ([c526139])
### Security
- fix(security): **IDOR — any user could overwrite any other user's event.** `tickets/update` accepts several event ids per request but authorized only `eventId`; `saleEventId`, `presaleEventId` and the `saleEventIds` values reached `prisma.event.update` with a bare `where: { id }`. Every id is now authorized up front in one batched query, so a partially-applied update isn't possible either. Deliberately reuses the app's existing write predicate (owner **or** collaborative-share editor) rather than an owner-only check, which would have broken legitimate shared editing — a test asserts a collaborative editor can still update ([1a2d3e4])
- fix(security): **SSRF — `tickets/refix` fetched a completely unvalidated URL**, taken from a user-settable event description, so storing `Ticket URL: http://169.254.169.254/…` made the server request cloud metadata. The guards on the two sibling scraping routes were also bypassable: no DNS resolution, no IPv6, no 169.254 block, and `fetch` follows redirects by default so an allowed URL could 302 to an internal target unchecked. New `src/lib/safe-fetch.ts` guards all three — protocol allowlist, DNS resolution rejecting private/loopback/link-local/CGNAT across v4 and v6 including IPv4-mapped forms, manual redirect handling that re-validates every hop, and a streamed 5 MB cap. `tickets/*` went from zero tests to 51 ([1a2d3e4])
- **Still open:** there is no rate limiting anywhere in the API, which compounds the credentials-login and password-change exposure. Needs a shared store (Upstash/Vercel KV) — a deployment decision, not a code one

### Added
- feat(calendar): hiding the `event-reminders` calendar now clears the category and location filters, which only narrow ticket-derived events — leaving them applied showed an empty calendar with no visible cause. Filters are snapshotted with the calendar list so a failed PUT restores both together, and the restore is skipped on load while the calendar is hidden (covers hiding it on one device and opening the app on another) ([90f7dea])
- feat(tickets): long AI scrapes are cancellable. There was no `AbortController` anywhere under `src/components/tickets`, so a hung 10-30s call left no recovery but a page reload — which also discarded the typed URL. Adds a Cancel button that preserves the URL, a 45s timeout with its own distinct message, a Retry action on the error card, and aggregate progress for the (still deliberately sequential) "Check all" ([b6253a3])

### Fixed
- fix(events): **the event modal silently destroyed data three ways** — delete fired immediately with no confirm or undo; toggling "All day" off recomputed start/end from the *current wall clock* rather than the user's times, so a mis-tap rewrote a real event; and Sync replaced the `event` prop, re-running the init effect and wiping unsaved edits. All three shared one cause (form state scattered across ~20 `useState`s plus a giant init effect), so they're fixed by extracting `useEventFormState`, which also gave the modal its first unsaved-changes guard. Undo re-POSTs and therefore does **not** preserve the original id — bookmarks and related-event links keyed on it aren't restored ([62592c9])
- fix(theme): the theme was applied in a post-mount effect, so every load for every user painted the default light theme and then flipped; OS-dark users saw a light→dark flash every visit. A `beforeInteractive` inline script now applies it during HTML parsing. Anti-drift is structural rather than disciplinary: the script embeds `applyThemeToDocument.toString()`, so there is no second implementation to keep in sync — verified against a production build that the minified function stays self-contained with no downlevel helper injected. Also fixes a stale-server-GET race that overwrote fresh theme choices, and a swallowed PUT failure ([dec42ec])
- fix(ui): ~8 mutations never checked `res.ok`, never rolled back, and never reported failure. Worst was `removeMember`, which dropped a collaborator from the list regardless of the response — a *failed access revocation* looked identical to a successful one. New `src/lib/mutate.ts` wrapper applied across calendar visibility, add/rename/delete/leave/duplicate/sync, share enable/disable and Google import ([d10712f])
- fix(ui): `WorldCupSection` had 16 raw colours and zero `dark:` variants, hardcoding `-400` shades as if always rendered dark. Retheme onto explicit light/dark `--wc-*` properties. Root cause of the wider class: no `--success` token existed, so every success message hand-rolls `green-500/600` — now added with contrast computed rather than eyeballed (5.19:1 light, 8.50:1 dark), which also caught the provisional-slot amber failing AA at 4.13:1 due to an opacity modifier ([52d8fe7])
- fix(reminders): reminders were tracked in `sessionStorage`, so they re-fired in every open tab and again after any reload. Moved to `localStorage` with `BroadcastChannel` cross-tab coordination and 24h pruning; lead time is now configurable instead of a hardcoded 10 minutes (settings UI to follow). Web push is deliberately not included ([0d61009])
- fix(worldcup): `LIVE_WINDOW_MS` was duplicated across two files and set to 2h, so a knockout match with extra time and penalties was marked "FT" while still being played — centralized, with a longer knockout window ([52d8fe7])

### Maintenance
- test: repaired the suite — was 59/75 with 6 of 9 files red, and 40 `tsc --noEmit` errors; now 81/81 across 10 files with tsc clean. Three independent causes: `tsconfig` never declared `vitest/globals` despite `test.globals: true` (36 errors, tests themselves fine); the route tests predated the calendar-sharing feature so mocks lacked `calendarMember` and the current `members`/`shareToken`/`shareMode` response shapes; and `worldcup-results.test.ts` still asserted 16 R32 matches after `VERIFIED_KNOCKOUT_TEAMS` was deliberately extended to the full 32-match bracket (73–104). `vi.mock` calls also moved out of the shared helper into each test file, since vitest hoists them per-file. No tests skipped, deleted, or weakened — ownership `where`-scoping expectations preserved, one error-message assertion now stricter, and no World Cup results invented ([d550ba6])
- docs: full-project audit (10 parallel review workers + 1 adversarial verifier) written up for review — 2 confirmed-exploitable vulnerabilities, build health, ~120 findings across UX, a11y, performance and design-system consistency. Report at `3-review/inbox/2026-07-26-event-calendar-full-audit.md` in the collaboration vault. **Open and unfixed:** the IDOR in `tickets/update` and the SSRF in `tickets/refix`

## [2026-07-20] — Session: RPD-aware AI model cascade
### Added
- feat(ai): the Gemini cascade is now aware of free-tier DAILY quotas (RPD) — the binding limit in practice: calls are tiny (~3k tokens) and spread out, so RPM/TPM never spike, but the ~20 req/day caps on flagship flash models ran out (AI Studio showed 31/20 on 3.5-flash, 22/20 on 2.5-flash-lite). New `AiModelUsage` table (migration `20260720000000_add_ai_model_usage`) keeps one global per-model counter keyed to Google's Pacific-midnight reset; every cascade (scrape, aiExtractJson, classify, description, grounded World Cup) now orders models by remaining daily headroom — models under an 80% soft reserve first in priority order, past-reserve models as last resort, exhausted ones skipped — so load spreads DOWN the cascade before any model hits its wall, and no doomed 429 is burned at the head of every call. A per-DAY 429 from Google marks the model spent until Pacific midnight ("trust Google's verdict"); if every model reads exhausted the full list is returned as a fallback (stale counts must not disable AI entirely)
- feat(ai): scrape-extraction cache — AI results are stored in a new `AiScrapeCache` table keyed by sha256(prompt version + url + extracted page text) with a 30-day TTL (migration `20260720000001_add_ai_scrape_cache`). Sync re-scrapes every ticket page on each check; when the page content is unchanged the prompt would be identical (temperature 0), so the stored result is reused at ZERO Gemini requests and without touching the user's daily app quota — verified the timable page text extracts byte-identical across fetches a day apart. Any content change or `EXTRACT_PROMPT_VERSION` bump changes the hash → fresh AI call. Follow-up enrichments (classify-fallback category, AI-composed description) are patched into the cache entry so repeat scrapes skip those calls too
- feat(ai): the extraction prompt now REQUIRES `description` and `category` (never null, composed from extracted facts) — previously a single scrape could cost up to 3 requests (extract + classify fallback + description fallback); one elaborated call now returns everything, with the fallbacks kept as rare safety nets
### Fixed
- fix(ai): `gemma-4-26b-it` does not exist (ListModels: real id is `gemma-4-26b-a4b-it`) — that cascade rung 404'd on every call; corrected. The Groq fallback's `llama3-8b-8192` is decommissioned and its 8k context couldn't fit a CJK-heavy page anyway — swapped to `llama-3.3-70b-versatile` (128k ctx); OpenAI-compatible fallback `max_tokens` raised 2048 → 4096 so many-window events don't truncate
- fix(ai): `parseJsonLoose` now tries balanced `{…}` blocks LAST-first and repairs truncations properly (drops a dangling partial member, closes unterminated strings and nested `{`/`[` with matching brackets) — gemma-4-26b-a4b-it leaks its reasoning as text that ECHOES the prompt's JSON schema as the first balanced block before emitting the real object last; first-block-wins would have returned the schema echo as data
### Maintenance
- docs(ai): models.ts documents why tempting idle-quota models must NOT be added (verified by live calls): the Live models (`gemini-3.1-flash-live-preview`, `gemini-3.5-live-translate-preview`) only support `bidiGenerateContent` (WebSocket Live API), and `antigravity-preview-05-2026` 400s with "This model only supports Interactions API" despite ListModels advertising generateContent

## [2026-07-19] — Session: reference URL sync source & manual data ops
### Added
- feat(bookmarks): bookmark popover pagination — 8 rows per page in date order (upcoming soonest-first, ended on the tail pages) with a Prev/Next footer + page indicator, shown only when needed; reopening resets to page 1
...
### Fixed
- feat(tickets): sale windows without an explicit close (a 1-day 預訂, a 公開發售 開始 row) now create/update their ⭐/🎫 reminder as a 1-day event (open → 23:59) instead of an arbitrary 2h/1h block; the Sync update path now also honours ranged windows' endDate/endTime, which it previously ignored ([d2c78c5])
- fix(tickets): same-day Timable sale windows (open and close on one day) show as a single on-sale moment in the ticket panel, not a redundant range ([e2f289c]); likewise open-ended 開始 rows whose Remix `endDatetime` is just the show day (Timable's "on sale until the event") are 1-day windows, not a range spanning months to the concert — only a close BEFORE the show (e.g. a lottery application window) counts as a real range
- fix(tickets): Timable sale windows now come from the page's Remix `ticketing[]` structured data (vendor name, 預訂/公開發售 type, exact HKT start–end) instead of the text-scan + AI guesswork — the 808 HI RECORDINGS scrape had shown "膠紙座" (the page's organizer channel) as the 預訂 vendor and dropped the Trip.com window entirely (the text scan's fixed vendor list missed hopestar/Trip.com and its date-only dedupe collapsed same-date vendors). When this complete source is present, AI platform/sale-window supplements are skipped ([99d0902])
- fix(tickets): the AI-prompt sentence splitter no longer severs domain-style vendor names mid-token ("Trip.com" → "Trip." + an orphaned sale row); prompt now also warns that a name beside 訂閱人數 subscriber counts is the organizer channel, not a sale vendor ([99d0902])
- fix(calendar): selecting the "Hong Kong" location chip no longer hides events whose location is written only in Chinese (e.g. "MOM Livehouse, 香港北角…") — the filter now matches title+location against the same `LOCATION_RULES` the sidebar counts use (shared `src/lib/location-tags.ts`), with substring match kept as fallback ([5239670])
- fix(ai): a stray trailing `}` in Gemini JSON-mode output (LLDream106 scrape) made the salvage parser return `{}` — the whole extraction (sale windows/prices/platforms) was silently dropped and Sync only showed meta-derived fields. `parseJsonLoose` now extracts the first balanced `{…}` block (string-aware brace scan); the scrape route's two inline parser copies now reuse it
- fix(ai): thinking-token starvation truncated AI extractions — gemini-3.5-flash spent ~1960 of the 2048 maxOutputTokens on hidden reasoning, leaving ~70 tokens of JSON (finishReason MAX_TOKENS); the salvaged fragment silently lost `saleDates`/`venueRuns` (title-only Pokémon RUN 30 scrape, missed 最速先行抽選 windows on Sync). Now `thinkingBudget: 0` for thinking-capable pool models (new `thinking` flag; Gemma skipped) + cap raised to 8192 — verified: RUN 30 → 9 venue runs, LLDream103 → 4 ranged lottery windows
### Maintenance
...
- feat(calendar): 📍 venue shown on events — all-day/banner events (e.g. six same-titled Bruno Mars venue runs), the list view, and week/day cells display the first segment of the location so tour stops are distinguishable on the grid
### Fixed
- fix(tickets): the AI prompt now extracts sale windows as start–end RANGES directly — "FC最速先行：10月13日(火)17:00〜10月18日(日)23:59" becomes one entry with date/time (open) + endDate/endTime (close), instead of only the open moment. Flows into the ranged display, ranged sale-reminder events, and year repair
- fix(tickets): repair AI-guessed years on sale dates — JP pages print sale windows without a year ("6月29日(月) 12:00〜"), so the AI invented one (2025 for a Jan-2027 tour). Deterministic fix bumps the year while the date is >60 days past and stays before the event (2025-06-29 → 2026-06-29); garbage dates (a 0570-… phone number parsed as year 570) are dropped. Prompt now also tells the model how to infer year-less sale dates
- fix(tickets): sale windows sharing one label (e.g. a lottery's open + close rows both titled オフィシャル1次抽選先行) merge into a single ranged window "2026-06-29 12:00 – 2026-07-11 23:59" — shown as a range in the ticket UI and created as ONE sale-reminder event spanning the application period instead of two duplicate reminders
...
- feat(worldcup): separate refresh buttons per stage — "Refresh group scores" on the Group Stage tab and "Refresh knockout scores" on the Road to Trophy tab (`?scope=group|knockout` on the API; a scoped refresh never drops the other stage's cached data)
- feat(worldcup): the knockout refresh now also **renames the calendar events** to real teams — R32 placeholders ("C組冠軍 vs F組亞軍") resolve immediately from the official map, and later rounds rename as winners are confirmed. A fixed `KNOCKOUT_FEEDERS` map is the bracket tree's source of truth, so renaming a title never breaks the structure. Runs on the button and the daily cron
- feat(worldcup): winner propagation — clicking "Refresh scores" advances the bracket. `propagateKnockout` fills each later-round slot ("M73勝者") with the team that actually won its feeder once that match is scored (penalty `winner` honoured), so the graph fills round-by-round as results land. The refresh also resolves and fetches scores for each newly-decided round, not just R32
...
- feat(worldcup): the score-refresh (B) now also fetches knockout Round-of-32 scores via AI — R32 fixtures are resolved to real teams from the standings, sent to the grounded model (numbered 1000+matchId), stored in the snapshot's `knockout[]`, and rendered in the bracket. Verified results (A) always override the AI snapshot (`mergeVerifiedKnockout`, applied in GET + POST). Deeper rounds resolve via the verified file until winner-propagation exists
...
### Changed
- feat(worldcup): the AI score prompt now points the model at the zh-yue knockout page (https://zh-yue.wikipedia.org/wiki/2026年FIFA世界盃淘汰賽) as the preferred knockout-results source; the Road to Trophy tab shows the "Updated · via <model>" badge too (previously only the Group Stage tab)
...
### Fixed
- fix(worldcup): correct the M75/M76 penalty winners — 德國 lost 3–4 to 巴拉圭 and 荷蘭 lost 2–3 to 摩洛哥 (I had both backwards). Winners now propagate correctly (M90 = 加拿大 vs 巴拉圭). Confirmed against the zh-yue knockout page
...
- fix(worldcup): add the verified Round-of-32 results played so far (M73 南非 0–1 加拿大, M74 巴西 2–1 日本, M75 德國 1–1 巴拉圭 4–3 pens, M76 荷蘭 1–1 摩洛哥 3–2 pens) to `VERIFIED_KNOCKOUT_SCORES`. The AI refresh was returning null for these, so scores never showed and winners couldn't advance; the verified layer now drives scores → dim-loser → winner propagation (M90 = 加拿大 vs 德國) → event renaming, deterministically
- fix(worldcup): lay out the knockout bracket by the real feeder tree instead of by match number — `buildBracket` now derives each match's vertical order and left/right half from the bracket tree (parsed from the "M73勝者" links, rooted at the Final), so matches that meet are adjacent and the two halves are the actual semifinal sub-trees (e.g. M81–84 now sit in the same half as M73–77). Matches the official wiki layout; falls back to match-number order when no tree is present
- fix(worldcup): correct the Round-of-32 bracket map — the seed's per-match-number slot mapping (from the EN article) put the wrong pairing under each number, so the R16 matchups were wrong. Added `VERIFIED_KNOCKOUT_TEAMS` (official zh-yue bracket, keyed by match number) that overrides the resolved R32 teams in the display and in AI score lookups (`getKnockoutTeams`). The R16→Final feeder tree was already correct
...
- feat(worldcup): pulsing "LIVE" badge + gleaming trophy on the Event Section's World Cup nav item during the tournament window ([cad6a1b])
